...
for event in app.stream(initial_state, stream_mode="updates"):
    # event 是一个字典，key 是节点名 或 "__start__"/"__end__"
    print("====== 新的节点更新 ======")
    for node_name, node_state_update in event.items():
        print(f"[节点] {node_name}")
        # 你关心 drafts / report / messages 等
        if "plan" in node_state_update:
            print("  plan.questions =", node_state_update["plan"].questions)
        if "drafts" in node_state_update:
            print("  drafts =")
            for i, d in enumerate(node_state_update["drafts"], 1):
                print(f"    子问题 {i} 的分析：{d[:80]}...")
        if "report" in node_state_update:
            print("  最终报告 前 200 字：")
            print(node_state_update["report"][:200], "...")